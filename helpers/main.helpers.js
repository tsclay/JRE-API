...
const FC = require('../FC')
const youTubeLinks = require('../youtube.FC')
...
main.get('/fc', async (req, res) => {
  try {
    ...
    // console.log(data.length, FC.length)

    // const goods = []

    // for (let i = 0; i < data.length; i++) {
    //   const youTube = moment(FC[i])
    //   const myDates = moment(data[i].date)

    //   if (
    //     data[i].episode_id !== 22 &&
    //     data[i].episode_id !== 36 &&
    //     data[i].episode_id !== 53 &&
    //     (youTube.format('MMMM D, YYYY') === myDates.format('MMMM D, YYYY') ||
    //       myDates.format('MMMM D, YYYY') ===
    //         youTube.add(1, 'days').format('MMMM D, YYYY'))
    //   ) {
    //     data[i].video_urls = [youTubeLinks[i]]
    //     // goods.push(data[i])
    //     Episode.updateOne({ _id: data[i]._id }, data[i], () => {
    //       console.log('update')
    //     })
    //     // console.log(i)
    //   } else {
    //     console.log(
    //       `${myDates.format('MMMM D, YYYY')} | ${youTube.format(
    //         'MMMM D, YYYY'
    //       )}`
    //     )
    //   }
    ...
    // const check = await Episode.find({
    //   isFC: true
    // }).sort({ date: 1 })
...
main.get('/all-1-299', async (req, res) => {
  try {
    const data = await Episode.aggregate([
      {
        $match: {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let j = 0; j < first.length; j++) {
        if (data[i].episode_id === first[j].episode_id) {
          data[i].video_urls.push(first[j].links)
        }
      }
    }
